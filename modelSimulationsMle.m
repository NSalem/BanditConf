...
addpath('helperfuncs\')
outfilename = 'model_simulations_resultsMLE_exp1.mat';
...
%                     this_ll = GetModelLL_QLearner(parametersLPP{isim,isub,igenmodel,irecmodel},modelsinfo{irecmodel},s,a,r,c,aa,ss,0);
                    ...
                    [parameters{isim,isub,igenmodel,irecmodel},this_ll,~,~,~,~,hessian]=fmincon(@(x) GetModelLL_QLearner(x,modelsinfo{irecmodel},a,r,0),x0,[],[],[],[],lb,ub,[],options);
%                     LPP(isim,isub,igenmodel,irecmodel) = this_LPP;
                    nfpm = numel(modelsinfo{irecmodel}.paramnames);
                    ll(isim,isub,igenmodel,irecmodel) = ll(isim,isub,igenmodel,irecmodel);
