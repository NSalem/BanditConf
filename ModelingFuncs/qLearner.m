...
%         
        ...
        function [action,p,conf,confUnchosen] = chooseAction(obj) 
            if isprop(obj,'T') && ~isempty(obj.T) 
                [action,p,conf,confUnchosen] = chooseActionThresholdSigmoid(obj);
            elseif isprop(obj,'lrv') && ~isempty(obj.lrv) 
                [action,p,conf,confUnchosen] = chooseActionThompson(obj);
            elseif isprop(obj,'beta') && ~isempty(obj.beta)
                [action,p,conf,confUnchosen]  = chooseActionSigmoid(obj); 
%             elseif isprop(obj,'sigma') && ~isempty(obj.sigma)
%                 [action,p,conf,confUnchosen]  = chooseActionFleming(obj);
            end
        end
        function [action,p,conf,confUnchosen] = chooseActionThompson(obj)
...
%            confUnchosen = (normpdf(Ex,-d,noise))/((normpdf(Ex,d,noise))+(normpdf(Ex,-d,noise)));
        ...
        function [action,p,conf,confUnchosen] = chooseActionSigmoid(obj)
                %Outputs:
                %action: (1 or 2)
